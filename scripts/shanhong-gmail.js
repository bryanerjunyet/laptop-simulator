const requiredMails = [
  {
    folder: "important",
    sender: "单鸿",
    from: "单鸿 <shanhong@wusha-studio.com>",
    to: "柳文清 <liuwenqing@wusha-gallery.com>",
    time: "2026.05.25 14:20",
    subject: "关于夏季主打展作品定案与推广策略",
    unread: true,
    snippet: "阿禾的作品先不要公开。柳文清的市场表现更好。",
    body: `文清，

夏季主打展的作品定案按今天下午会议的方向执行。

阿禾的作品先不要公开。柳文清的市场表现更好，媒体与藏家对你的名字已经有基础认知。我们不需要重新教育市场，也不需要把一个还不稳定的幕后画工推到台前。

《雨后的第七盏灯》先进入你的展线。构图、笔触与原稿保持七成一致，色调和标题说明由展厅统一处理。预展登记以你的名字走，不要让外部看到 A.H. 的原始创建记录。

登记表不要出现阿禾姓名。工作室编号可以保留内部追溯，但对外资料只使用你的签名。佣金部分等成交后再按内部批注拆分。

这不是讨论，是推广策略。`
  },
  {
    folder: "important",
    sender: "单鸿",
    from: "单鸿 <shanhong@wusha-studio.com>",
    to: "阿禾 <ahe@oldstudio.mail>",
    time: "2026.06.15 13:30",
    subject: "Re: 展厅作品《雨后的第七盏灯》署名更正申请",
    unread: true,
    snippet: "申请不予通过。你无法证明该作品具备独立创作权。",
    body: `阿禾，

你于 2026.06.15 11:30 提交的《雨后的第七盏灯》署名更正申请，画廊已完成初步审阅。

申请不予通过。

理由如下：

1. 你无法证明该作品具备独立创作权。
2. 展厅现有版本已经经过画廊商业化调整，与原始草稿并非同一展示文件。
3. 柳文清作为签约画师，已完成正式登记及授权流程。

请不要再以个人情绪影响展厅运营。若继续向外部散布未经证实的信息，画廊将保留追究责任的权利。

单鸿

----- 原始邮件 -----
发件人：阿禾 <ahe@oldstudio.mail>
时间：2026.06.15 11:30

《雨后的第七盏灯》是我在 3 月 12 日完成底稿的作品。预展资料把作者写成柳文清，这是错误的。

请立刻更正署名，并停止使用我的草稿编号。`
  },
  {
    folder: "important",
    sender: "阿禾",
    from: "阿禾 <ahe@oldstudio.mail>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    cc: "画廊管理层 <gallery-admin@wusha-studio.com>",
    time: "2026.06.22 23:40",
    subject: "正式投诉：请立刻停止对本人作品的侵权与冒名",
    unread: true,
    snippet: "这些作品不是柳文清的……你们不能把我的名字拿走。",
    body: `单先生：

这是正式投诉。

这些作品不是柳文清的。《雨后的第七盏灯》的底稿、构图、修改记录都来自我的工作文件，3 月 12 日的原始文件、右下角日期刻痕和工作室编号都能对上。你们不能把我的名字拿走。

我从来没有同意把作品登记到柳文清名下，也没有同意任何所谓“幕后创作者”的安排。

请立刻停止展售相关作品，恢复真实署名，并公开更正已经发出的展厅资料。

如果画廊继续回避，我会向外部机构提交我保存的草稿、日期记录与创作文件。我不是要闹事，我只是要求作品回到真实作者名下。

阿禾`
  },
  {
    folder: "important",
    sender: "单鸿",
    from: "单鸿 <shanhong@wusha-studio.com>",
    to: "画廊内部人员 <gallery-team@wusha-studio.com>",
    time: "2026.07.05 16:00",
    subject: "【内部统一口径】针对近期某常驻画工争议言论的应对提纲",
    unread: false,
    snippet: "统一使用“炒作”“长期模仿柳文清”等表述。",
    body: `各部门：

近期如有媒体或合作方询问某常驻画工对署名问题的争议，请统一按照以下口径回应：

一、该人员长期在画廊学习、临摹成熟签约画师风格，作品尚不具备独立市场辨识度。

二、所谓“署名争议”属于个人情绪化表达，不排除借柳文清热度炒作。

三、画廊尊重原创，也尊重成熟商业流程。所有展出作品均完成内部登记。

四、任何员工不得向外提供草稿、登记编号、佣金表、内部评估文件截图。若被问到 3 月份创作时间差，一律回答为“内部整理日期不代表创作日期”。

采访中避免提及“阿禾”姓名，统一称“某常驻画工”。`
  },
  {
    folder: "important",
    sender: "单鸿",
    from: "单鸿 <shanhong@wusha-studio.com>",
    to: "合作媒体 / 营销号 <media-list@wusha-pr.com>",
    time: "2026.08.02 10:00",
    subject: "【独家发布】关于某小众画手抄袭柳文清作品之证据链对比",
    unread: true,
    snippet: "请在 11:30 前后同步发布，标题可以直接指向抄袭争议。",
    attachment: "对比证据_媒体报道版.png",
    body: `各位，

附件为本次争议的对比图，请在 11:30 前后同步发布。

标题可以直接指向“年轻画家涉嫌抄袭知名画家柳文清”。重点放在构图相似、笔触近似、当事人近期频繁申诉等角度。

不需要展开画廊内部登记流程。公众只需要看到一条清楚的线：柳文清在前，小众画手在后，对方借争议碰瓷。

图片角落细节无需放大，按整图使用即可。不要单独截取草稿右下角，也不要讨论原图来源，避免把问题引回画廊内部登记。

单鸿`
  }
];

const extraMails = [
  {
    folder: "inbox",
    sender: "五煞文创行政部",
    from: "五煞文创行政部 <admin@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.08.01 18:10",
    subject: "周年庆典发布会流程确认",
    unread: false,
    snippet: "中央塔基灯光、直播机位与贵宾动线已更新。",
    body: `单总，

周年庆典发布会流程已按您要求更新。

中央塔基灯光将在续封仪式开始前十分钟预热，直播机位改为正面主机位加两侧补位，贵宾动线不经过画廊后仓区域。

画廊部分只保留柳文清的公开作品介绍，不安排阿禾或其他幕后人员露面。`
  },
  {
    folder: "inbox",
    sender: "财务室",
    from: "财务室 <finance@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.05.30 09:25",
    subject: "《雨后的第七盏灯》成交款项拆分表",
    unread: true,
    snippet: "成交价 RM80,000，关联账户分配请按批注处理。",
    body: `单总，

《雨后的第七盏灯》已完成成交登记，成交价 RM80,000。

款项拆分按批注处理：柳文清 RM20,000，画廊账户 RM20,000，您指定的关联账户 RM40,000。作品编号仍沿用原工作室编号，登记作者为柳文清。

如需调整发票备注，请在今晚前回复。`
  },
  {
    folder: "inbox",
    sender: "展厅运营",
    from: "展厅运营 <gallery-ops@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.06.01 10:42",
    subject: "匿名展览申请：《没人看见的东西》",
    unread: false,
    snippet: "申请人备注：不售卖，只希望让真正喜欢的人看见。",
    body: `单总，

收到一份匿名展览申请，展名为《没人看见的东西》，申请展期 2026.07.01 - 2026.07.07。

申请人备注写着：“不售卖，只希望让真正喜欢的人看见。”

作品风格、笔触和近期争议作品接近，运营部怀疑申请人与阿禾有关。是否直接驳回，还是先暂存不回复？`
  },
  {
    folder: "inbox",
    sender: "法务顾问",
    from: "法务顾问 <legal@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.07.08 19:18",
    subject: "关于投诉人报警后的应对建议",
    unread: false,
    snippet: "如警方未立案，暂不主动回应。",
    body: `单先生，

关于阿禾报警称作品被盗一事，目前反馈是证据不足，暂未立案。

建议画廊不要主动回应具体创作时间、原稿编号、佣金分配及登记流程。若对方继续公开发声，可继续使用“署名争议为个人主张，画廊已完成正式登记”的表述。

请内部人员避免留下新的书面确认。`
  },
  {
    folder: "inbox",
    sender: "王充奇",
    from: "王充奇 <wangchongqi@wusha-gallery.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.08.18 03:06",
    subject: "作品记录不应该被删",
    unread: true,
    snippet: "如果那些东西是真的，为什么不能留下？",
    body: `单鸿，

我看见后仓那几个箱子被重新贴了标签。

如果那些东西是真的，为什么不能留下？草稿、原稿、修改记录，本来就是证明一幅画怎样诞生的东西。

你可以说这是商业流程，但不要把“真实”也当成可以处理掉的库存。`
  },
  {
    folder: "inbox",
    sender: "仓储",
    from: "仓储 <storage@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.08.23 21:50",
    subject: "版权争议处理方案：原稿箱移交确认",
    unread: false,
    snippet: "剩余草稿及修改记录已按版权争议处理方案封存。",
    body: `单总，

剩余草稿、原稿及修改记录已按版权争议处理方案封存，箱号为 G-07、G-08。

目前箱内文件仍可追溯至阿禾的原始工作室编号。若确认执行销毁，请回复书面指令后由仓储两人共同登记。

备注：部分纸本右下角有手写日期与 A.H. 标记。`
  },
  {
    folder: "inbox",
    sender: "五煞文创人事",
    from: "五煞文创人事 <hr@wusha-studio.com>",
    to: "单鸿 <shanhong@wusha-studio.com>",
    time: "2026.08.12 08:30",
    subject: "工作证补办记录与昨夜临时出入记录",
    unread: false,
    snippet: "昨夜临时出入记录已导出。",
    body: `单总，

昨夜画廊后仓临时出入记录已导出。监控中能看到一名人员进入后仓，只搬走一整箱画稿，袖口露出五煞文创工作证。

由于画面角度无法确认姓名，人事部只能先核对近期工作证补办记录。相关表格已放入内部盘。`
  }
];

const folders = {
  inbox: [...requiredMails, ...extraMails],
  important: requiredMails,
  sent: requiredMails.filter((mail) => mail.sender === "单鸿"),
  trash: []
};

const list = document.getElementById("mailList");
const reader = document.getElementById("reader");
const gmailApp = document.querySelector(".gmail-app");
const gmailMenuButton = document.getElementById("gmailMenuButton");
const gmailSearchForm = document.getElementById("gmailSearchForm");
const mailSearchInput = document.getElementById("mailSearchInput");
const mailRange = document.querySelector(".mail-toolbar > span");
const toolbarCheckboxButton = document.querySelector(".toolbar-checkbox-button");
const mailMoreButton = document.getElementById("mailMoreButton");
const mailMoreMenu = document.getElementById("mailMoreMenu");
const gmailProfileButton = document.getElementById("gmailProfileButton");
const gmailProfileMenu = document.getElementById("gmailProfileMenu");
let activeFolder = "inbox";
let activeVisibleMails = folders.inbox;
const attachmentImages = {
  "对比证据_媒体报道版.png": "../assets/images/evidence/ahe-media-comparison.png"
};

gmailMenuButton.addEventListener("click", () => {
  gmailApp.classList.toggle("is-sidebar-collapsed");
});

gmailSearchForm.addEventListener("submit", (event) => {
  event.preventDefault();
  openFolder(activeFolder);
});

mailSearchInput.addEventListener("input", () => {
  openFolder(activeFolder);
});

toolbarCheckboxButton.addEventListener("click", () => {
  toolbarCheckboxButton.classList.toggle("is-selected");
});

mailMoreButton.addEventListener("click", (event) => {
  event.stopPropagation();
  gmailProfileMenu.classList.add("is-hidden");
  mailMoreMenu.classList.toggle("is-hidden");
});

mailMoreMenu.addEventListener("click", (event) => {
  event.stopPropagation();
});

gmailProfileButton.addEventListener("click", (event) => {
  event.stopPropagation();
  mailMoreMenu.classList.add("is-hidden");
  gmailProfileMenu.classList.toggle("is-hidden");
});

gmailProfileMenu.addEventListener("click", (event) => {
  event.stopPropagation();
});

document.addEventListener("click", () => {
  gmailProfileMenu.classList.add("is-hidden");
  mailMoreMenu.classList.add("is-hidden");
});

document.querySelectorAll("[data-folder]").forEach((button) => {
  button.addEventListener("click", () => {
    activateFolder(button.dataset.folder);
    openFolder(button.dataset.folder);
  });
});

function openFolder(folder) {
  if (folder === "compose") {
    openCompose();
    return;
  }

  gmailApp.classList.remove("is-reading");
  activeFolder = folder;
  const baseMails = folders[folder] || [];
  const query = mailSearchInput.value.trim().toLowerCase();
  const mails = query
    ? baseMails.filter((mail) => mailMatchesSearch(mail, query))
    : baseMails;
  activeVisibleMails = mails;
  updateMailRange(mails.length, baseMails.length);

  if (!mails.length) {
    list.innerHTML = "";
    reader.innerHTML = `<div class="empty"><div><h2>No mail here</h2><p>${query ? "No matching messages." : "This folder has no messages."}</p></div></div>`;
    return;
  }

  list.innerHTML = mails.map((mail, index) => `
    <article class="mail-row ${mail.unread ? "unread" : ""} ${index === 0 ? "active" : ""}" data-index="${index}" role="button" tabindex="0">
      <button class="row-check" type="button" aria-label="Select mail"></button>
      <span class="row-star ${mail.folder === "important" ? "is-starred" : ""}" aria-hidden="true">${mail.folder === "important" ? "★" : "☆"}</span>
      <span class="mail-copy">
        <span class="sender">${mail.sender}</span>
        <span class="subject">${mail.subject} <em>${mail.snippet}</em></span>
      </span>
      <span class="time">${mail.time.slice(5)}</span>
    </article>
  `).join("");

  list.querySelectorAll(".mail-row").forEach((row) => {
    row.addEventListener("click", () => {
      selectMailRow(row);
    });

    row.addEventListener("keydown", (event) => {
      if (event.key !== "Enter" && event.key !== " ") return;
      event.preventDefault();
      selectMailRow(row);
    });
  });

  list.querySelectorAll(".row-check").forEach((checkbox) => {
    checkbox.addEventListener("click", (event) => {
      event.stopPropagation();
      checkbox.classList.toggle("is-selected");
    });
  });

  renderMail(mails[0]);
}

function selectMailRow(row) {
  list.querySelectorAll(".mail-row").forEach((item) => item.classList.remove("active"));
  row.classList.add("active");
  gmailApp.classList.add("is-reading");
  renderMail(activeVisibleMails[Number(row.dataset.index)]);
}

function mailMatchesSearch(mail, query) {
  return [
    mail.sender,
    mail.from,
    mail.to,
    mail.cc,
    mail.time,
    mail.subject,
    mail.snippet,
    mail.body,
    mail.attachment
  ].filter(Boolean).join(" ").toLowerCase().includes(query);
}

function updateMailRange(visibleCount, totalCount) {
  if (!mailRange) return;
  mailRange.textContent = visibleCount
    ? `1-${visibleCount} of ${totalCount}`
    : `0-0 of ${totalCount}`;
}

function openCompose() {
  list.innerHTML = "";
  gmailApp.classList.add("is-reading");
  reader.innerHTML = `
    <button class="compact-back-mail" type="button" id="compactBackCompose">Back</button>
    <form class="compose-panel compose-form" id="composeForm">
      <header>New Message</header>
      <input type="email" placeholder="To">
      <input type="text" placeholder="Subject">
      <textarea placeholder="Compose email"></textarea>
      <footer>
        <button type="submit">Send</button>
      </footer>
    </form>
  `;

  document.getElementById("composeForm").addEventListener("submit", (event) => {
    event.preventDefault();
    activateFolder("inbox");
    openFolder("inbox");
  });

  document.getElementById("compactBackCompose").addEventListener("click", () => {
    activateFolder(activeFolder);
    openFolder(activeFolder);
  });
}

function activateFolder(folder) {
  document.querySelectorAll("[data-folder]").forEach((item) => {
    item.classList.toggle("active", item.dataset.folder === folder);
  });
}

function renderMail(mail) {
  const paragraphs = mail.body.split("\n\n").map((paragraph) => {
    if (paragraph.startsWith("-----")) {
      return `<blockquote>${paragraph.replaceAll("\n", "<br>")}</blockquote>`;
    }
    return `<p>${paragraph.replaceAll("\n", "<br>")}</p>`;
  }).join("");

  reader.innerHTML = `
    <button class="compact-back-mail" type="button" id="compactBackMail">Back</button>
    <h1>${mail.subject}</h1>
    <div class="meta">
      <strong>${mail.from}</strong><br>
      <span>to ${mail.to}${mail.cc ? ` · cc ${mail.cc}` : ""} · ${mail.time}</span>
    </div>
    ${paragraphs}
    ${mail.attachment ? `<button class="attachment" type="button" data-attachment="${mail.attachment}">${mail.attachment}</button>` : ""}
  `;

  reader.querySelectorAll("[data-attachment]").forEach((button) => {
    button.addEventListener("click", () => openAttachment(button.dataset.attachment));
  });

  document.getElementById("compactBackMail").addEventListener("click", () => {
    gmailApp.classList.remove("is-reading");
  });
}

function openAttachment(attachmentName) {
  const image = attachmentImages[attachmentName];
  if (!image) return;

  reader.innerHTML = `
    <button class="back-to-mail" type="button" id="backToMail">Back</button>
    <article class="attachment-viewer">
      <header>
        <h1>${attachmentName}</h1>
        <p>Image preview</p>
      </header>
      <img src="${image}" alt="${attachmentName}">
    </article>
  `;

  document.getElementById("backToMail").addEventListener("click", () => {
    const activeRow = list.querySelector(".mail-row.active");
    renderMail(activeVisibleMails[Number(activeRow?.dataset.index || 0)] || activeVisibleMails[0]);
  });
}

openFolder("inbox");
